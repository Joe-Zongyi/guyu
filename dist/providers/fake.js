...
function findTaxonomyByHint(input) {
    const lc = hint(input);
    for (const entry of TAXONOMY_CATALOG) {
        if (lc.includes(entry.taxonomy_id))
            return entry;
        if (lc.includes(entry.common_name.toLowerCase()))
            return entry;
...
export class FakeVisionProvider {
    ...
    async identifyPlant(req) {
        ...
        if (tag.includes("multi"))
            return { kind: "multiple_plants" };
        if (tag.includes("ambiguous")) {
            const a = findTaxonomyByHint(tag) ?? TAXONOMY_CATALOG[0];
            const b = TAXONOMY_CATALOG.find((e) => e.taxonomy_id !== a.taxonomy_id) ??
                TAXONOMY_CATALOG[1];
            return {
                kind: "identified",
                candidates: [
    ...
    async assessPlantState(req) {
        ...
        const matchedSignals = SIGNAL_HINTS.filter((h) => tag.includes(h.key)).map((h) => ({ signal: h.signal, confidence: 0.8 }));
        if (tag.includes("lowconf") || tag.includes("low_conf")) {
            return {
        ...
        return {
            ...
            overall_confidence: Math.min(...matchedSignals.map((s) => s.confidence)),
            image_quality: "ok",
            detected_plant: true,
        };
    }
